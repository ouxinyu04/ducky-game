...
class Duck:
    def __init__(self, x, y):
        ...
        self.frame_time = 200  # 200ms per frame
        self.last_frame_update = 0

        # 加载静止动画 (ducky_animation_144.gif)
        self.idle_frames = []
        self.load_idle_animation()

        # 加载行走动画 (ducky-walk.gif)
        self.walk_frames = []
        self.walk_frames_mirrored = []
        self.load_walk_animation()

        # 备用图像
        self.png_image = None
        self.load_backup_image()

    def load_idle_animation(self):
        """加载静止动画 (ducky_animation_96.gif)"""
        idle_path = os.path.join(ASSETS_DIR, "ducky_animation_96.gif")
        if os.path.exists(idle_path):
            try:
                gif = Image.open(idle_path)
                for frame in range(gif.n_frames):
                    ...
                    self.idle_frames.append(pygame_image)
                logging.info(f"Loaded {len(self.idle_frames)} idle animation frames")
            except Exception as e:
                logging.error(f"Failed to load idle animation: {e}")

        if not self.idle_frames:
            logging.warning("No idle animation frames loaded, using fallback")

    def load_walk_animation(self):
        """加载行走动画 (ducky-walk-96.gif) 和其镜像版本"""
        walk_path = os.path.join(ASSETS_DIR, "ducky-walk-96.gif")
        if os.path.exists(walk_path):
            try:
                gif = Image.open(walk_path)
                for frame in range(gif.n_frames):
                    gif.seek(frame)
                    frame_image = gif.convert("RGBA")
                    ...
                    self.walk_frames.append(pygame_image)

                    # 创建镜像版本（左右翻转）
                    mirrored_image = pygame.transform.flip(pygame_image, True, False)
                    self.walk_frames_mirrored.append(mirrored_image)

                logging.info(f"Loaded {len(self.walk_frames)} walk animation frames (+ mirrored)")
            except Exception as e:
                logging.error(f"Failed to load walk animation: {e}")

        if not self.walk_frames:
            logging.warning("No walk animation frames loaded, using fallback")

    def load_backup_image(self):
        """加载备用图像"""
        png_path = os.path.join(ASSETS_DIR, "duck.gif")
    ...
    def draw(self, screen):
        if self.is_alive:
            ...
            if current_time - self.last_frame_update >= self.frame_time:
                ...
                # 根据移动状态选择动画帧列表
                if self.is_moving:
                    if self.is_moving_left and self.walk_frames_mirrored:
                        # 使用镜像行走动画
                        frame_list = self.walk_frames_mirrored
            ...
            if frame_list:
                screen.blit(frame_list[self.current_frame], (self.x, self.y))
            elif self.png_image:
    ...
    def move(self, keys):
        ...
        self.is_moving_left = moving_left

        # 如果状态改变，重置动画帧
        if moving != getattr(self, '_prev_moving', False):
            self.current_frame = 0
